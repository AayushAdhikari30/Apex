...
import {
  ...
  LockedFeature,
  useGate,
} from "@/components/monetization/FreemiumGate";
import {
  PostAddAffiliate,
  RevenueTracker,
} from "@/components/monetization/AffiliateLinks";
...
export default function App() {
  ...
  const pageMap = {
    ...
    stress: <StressPage />,
    monte: (
      <LockedFeature feature="Monte Carlo Simulation">
        <MonteCarloPage />
      </LockedFeature>
    ),
    tax: (
      <LockedFeature feature="Tax Engine">
        <TaxPage />
      </LockedFeature>
    ),
    ...
    revenue: (
      <div className="animate-fade-up">
        <div className="font-display text-[20px] font-bold italic mb-1">
          Revenue Tracker
        </div>
        <div className="font-mono text-[10px] text-[#3d4a6b] mb-4">
          Your affiliate and subscription earnings
        </div>
        <div className="bg-[#0f1326] border border-[#1a2035] rounded-xl p-5">
          <RevenueTracker />
        </div>
      </div>
    ),
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      />
      {showAffPrompt && lastAddedSym && (
        <PostAddAffiliate
          sym={lastAddedSym}
          onClose={() => setShowAffPrompt(false)}
        />
      )}
    </div>
  );
